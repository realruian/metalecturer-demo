...
本项目的原创代码及配套文档采用根目录 `LICENSE` 中的 MIT 许可证，包括 `app/`、`components/`、`lib/`、`tests/` 中的原创代码及项目配置。代码中的第三方产品名称和标识不因此获得额外授权。

以下内容不属于本项目 MIT 许可证授予的范围：

- `public/assets/` 中的图片与图标素材，包括原型提取图片、生成图片及包含第三方产品标识的课程封面。
- `public/media/` 中的讲解音频。
- `docs/screenshots/` 中包含上述素材的界面截图。
- 第三方依赖及其自带的代码、文档和素材，其许可仍由各自的权利人提供。

这些素材的来源记录在下文；来源记录本身不构成额外的使用授权。本次代码开源不替代素材原有许可，也不代表已确认所有素材的再分发或商业使用权限。
